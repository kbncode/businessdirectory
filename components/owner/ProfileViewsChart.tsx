...
import type { DailyViewPoint } from "@/lib/queries/owner-dashboard";

// Hex values match tailwind.config's ink/paper/stone/sand/signalOrange —
// recharts styling props take literal colors, not Tailwind classes.
export function ProfileViewsChart({ points }: { points: DailyViewPoint[] }) {
  return (
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={points} margin={{ top: 8, right: 12, left: -16, bottom: 0 }}>
          ...
          />
          <Line type="monotone" dataKey="views" name="Views" stroke="#F06826" strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
