...
// Every stored viewDate/eventDate is already truncated to midnight UTC (see
// lib/visitor.ts's startOfUtcDay, used by the tracking routes) — ranges here
// are resolved the same way so a plain [gte start, lte end] comparison lines
// up exactly, with no off-by-one from timezone drift.
